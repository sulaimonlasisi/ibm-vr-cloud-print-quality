...
visual_recognition = VisualRecognitionV3('2016-05-20', api_key='8bee4e797b67e1b223f6eba317a37d1eb8a0ee7f')


# with open(join(dirname(__file__), '../resources/cars.zip'), 'rb') as cars, \
#        open(join(dirname(__file__), '../resources/trucks.zip'), 'rb') as
# trucks:
#     print(json.dumps(virtual_recognition_instance.create_classifier('Cars vs Trucks',
...
with open(car_path, 'rb') as images_file:
    car_results = virtual_recognition_instance.classify(images_file=images_file,
                                              threshold=0.1,
                                              classifier_ids=[
...
# print(json.dumps(virtual_recognition_instance.get_classifier('YOUR CLASSIFIER ID'),
# indent=2))
